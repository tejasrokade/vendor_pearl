...
func init() {
	android.RegisterModuleType("pearl_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = pearlExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = pearlExpandVariables(ctx, dep_file)
	...
	cmd := pearlExpandVariables(ctx, String(g.properties.Cmd))
